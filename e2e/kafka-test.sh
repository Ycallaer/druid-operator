echo "Creating Task"
task_id=$(curl -s -X 'POST' -H 'Content-Type:application/json' -d @kafka-test-config.json http://druid-tiny-cluster-coordinators.druid.svc:8088/druid/indexer/v1/task | jq '.task' -r)
echo $task_id
if [ $? == 0 ]
then 
